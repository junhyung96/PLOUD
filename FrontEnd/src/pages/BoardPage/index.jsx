...
const BoardPage = () => {
  ...
  const handleSearch = async () => {
    if (searchForm.trim() === "") {
      ...
      try {
        const response = await searchBoard(token, {
          title: searchForm,
          page: 0,
          size: 10,
          sort: "title",
      ...
      <Page header={<Navbar />} footer={<Footer />}>
        ...
        <div className="OutBox rounded-md outline2 mx-auto mt-7">
          <div className="study-main-search flex justify-center mb-3">
            ...
            />
            <button
              className="border searchBtn p-1 ms-3 rounded-md bg-sky-300 writeBtn"
              onClick={handleSearch}>
              검색
            </button>
            <button
              className="write border writeBtn rounded-md py-1 px-4 ml-6"
              onClick={() =>
                navigate("/createboard", {
                  state: { isCreate: true, boardId: -1 },
                })
              }>
              글쓰기
            </button>
          ...
          <table className="table-auto text-center bg-white h-44">
            <thead className="underLine">
              <tr>
                <th className="w-6" style={{ backgroundColor: "#EBEAFA" }}>
                  No
                </th>
                <th className="w-56" style={{ backgroundColor: "#EBEAFA" }}>
                  제목
                </th>
                <th className="w-6" style={{ backgroundColor: "#EBEAFA" }}>
                  작성자
